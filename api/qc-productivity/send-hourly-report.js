...
async function generateReportImageBuffer(data, reportDate, hourStart, hourEnd, thresholdsMap) {
            ...
            <div style={{
                ...
                <div style={{
                    ...
                    <p style={{ color: '#64748b', fontSize: '16px', margin: '0 0 20px 0' }}>
                        👥 Nhân viên có dữ liệu: {rows.length} &nbsp;|&nbsp; ⏰ {hourStart}h - {hourEnd}h
                    </p>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', width: '100%' }}>
                        ...
                        <div style={{
                            ...
                            <div style={{ width: '100px', padding: '14px', textAlign: 'center' }}>Tổng SL</div>
                        </div>
                        {/* Rows */}
                        {rows.map((row) => (
                            <div style={{
                                ...
                                <div style={{ width: '50px', padding: '14px', color: '#1e293b' }}>{row.index + 1}</div>
                                <div style={{ flex: 2, padding: '14px', display: 'flex', alignItems: 'center', gap: '12px' }}>
                                    {/* Avatar tròn */}
                                    <div style={{
                                        width: '36px',
                                        height: '36px',
                                    ...
                                    <div>
                                        <div style={{ fontWeight: 600, color: '#1e293b' }}>{row.displayName}</div>
                                        <div style={{ fontSize: '12px', color: '#64748b' }}>{row.email}</div>
                                    </div>
                                </div>
                                <div style={{ flex: 1, padding: '14px', color: '#1e293b' }}>{row.role}</div>
                                <div style={{
                                    width: '100px',
                                    padding: '14px',
                                    textAlign: 'center',
...
export default async function handler(req, res) {
    ...
    try {
        ...
        try {
            // Tạo ảnh buffer
            const imageBuffer = await generateReportImageBuffer(processedData, reportDate, hourStart, hourEnd, thresholdsMap);
