...
export default async function handler(req, res) {
    try {
        ...
        const filteredUsers = activeUsers.filter(user => !excludedEmails.includes(user.email));
        ...
        // Chỉ xét những user đã có log hôm nay
        ...
        // Lấy danh sách user đang có assignment active (thời điểm hiện tại nằm trong khoảng)
        ...
        const activeAssignmentSet = new Set((activeAssignments || []).map(a => a.user_email));

        // Tính idle cho từng user đã có log
        const idleUsers = [];
        for (const user of usersWithLogs) {
            // Bỏ qua nếu user đang trong assignment
            if (activeAssignmentSet.has(user.email)) continue;
            ...
            // Cooldown: nếu có log mới sau lần cảnh báo trước thì reset
        ...
        // QUAN TRỌNG: Nếu tất cả người đã log đều idle -> không còn ai làm -> dừng cảnh báo
