...
export function calcIdleBetweenLogs(
    ...
    const effectiveLast = Math.max(lastLogMs, workStartMs);
    const effectiveCurr = Math.min(currentLogMs, workEndMs);
    if (effectiveCurr <= effectiveLast) return 0;

    let idleMs = effectiveCurr - effectiveLast;

    if (breakStartMs && breakEndMs && effectiveLast < breakEndMs && effectiveCurr > breakStartMs) {
        const overlapStart = Math.max(effectiveLast, breakStartMs);
        const overlapEnd = Math.min(effectiveCurr, breakEndMs);
        if (overlapEnd > overlapStart) {
            idleMs -= overlapEnd - overlapStart;
        }
    }
...
/**
 * Lấy assignments trong khoảng ngày cho danh sách email (nếu có)
 * @param {object} supabase
 * @param {string} dateStr 'YYYY-MM-DD'
 * @param {string[]} [userEmails] - mảng email, nếu không truyền thì lấy tất cả
 * @returns {Map<string, Array<{start: number, end: number, reason: string}>>}
 */
export async function fetchAssignmentsForDate(supabase, dateStr, userEmails = null) {
    const startOfDay = `${dateStr}T00:00:00+07:00`;
    const endOfDay = `${dateStr}T23:59:59+07:00`;

    let query = supabase
        .from('qc_work_assignments')
        .select('user_email, start_time, end_time, reason')
...
/**
 * Tính thời gian overlap (ms) giữa khoảng [a, b] và các assignment
 */
export function calcAssignmentOverlap(a, b, assignments) {
    if (!assignments || assignments.length === 0) return 0;
    let overlapMs = 0;
    for (const asgn of assignments) {
        const overlapStart = Math.max(a, asgn.start);
        const overlapEnd = Math.min(b, asgn.end);
        if (overlapEnd > overlapStart) {
...
// --- Cập nhật các hàm tính idle để nhận tham số assignments ---

/**
 * Cập nhật calcIdleMinutes (dùng cho alert) - thêm tham số assignments = []
 */
export function calcIdleMinutes(
    lastLogMs,
    nowMs,
    ...
    // Trừ thời gian assignment
    idleMs -= calcAssignmentOverlap(start, end, assignments);

    return Math.max(0, Math.floor(idleMs / 60000));
}

/**
 * Cập nhật calcIdleBetweenLogs (dùng cho report) - thêm tham số assignments = []
 */
export function calcIdleBetweenLogs(
    lastLogMs,
    currentLogMs,
    ...
    // Trừ assignment
    idleMs -= calcAssignmentOverlap(effectiveLast, effectiveCurr, assignments);

    return Math.max(0, Math.floor(idleMs / 60000));
}

/**
 * Tính idle ngoài khung log (đầu/cuối ca) có trừ assignment
 */
export function calcEdgeIdle(firstLogMs, lastLogMs, workStartMs, workEndMs, assignments = []) {
    let edgeIdle = 0;
    if (firstLogMs && firstLogMs > workStartMs) {
        let idleMs = firstLogMs - workStartMs;
        idleMs -= calcAssignmentOverlap(workStartMs, firstLogMs, assignments);
        edgeIdle += Math.max(0, idleMs);
    }
    if (lastLogMs && lastLogMs < workEndMs) {
        let idleMs = workEndMs - lastLogMs;
        idleMs -= calcAssignmentOverlap(lastLogMs, workEndMs, assignments);
        edgeIdle += Math.max(0, idleMs);
    }
    return Math.max(0, Math.floor(edgeIdle / 60000));
}