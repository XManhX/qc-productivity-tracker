...
async function getReportConfig() {
    const alertConfig = await getAlertConfig(supabase); // từ helpers
    // Merge với default report config, ưu tiên giá trị từ DB nếu có
    return {
        ...DEFAULT_REPORT_CONFIG,
        // Lấy các trường báo cáo từ alert config (nếu có)
        ...
        // Các thông tin giờ làm, break, excluded vẫn từ alertConfig
...
// ========== TÍNH IDLE ĐẦU CA VÀ CUỐI CA ==========
/**
 * Tính idle ngoài khung log (đi muộn, về sớm)
 * @returns {number} số phút idle thêm
 */
function calcEdgeIdle(firstLogMs, lastLogMs, workStartMs, workEndMs) {
    let edgeIdle = 0;
    if (firstLogMs && firstLogMs > workStartMs) {
        edgeIdle += firstLogMs - workStartMs;
    }
    if (lastLogMs && lastLogMs < workEndMs) {
        edgeIdle += workEndMs - lastLogMs;
    }
...
async function getDailyReport(dateStr, skipCache = false) {
    ...
    for (const [email, stats] of userMap) {
        ...
        // Tìm log đầu, log cuối (nếu có từ 2 log trở lên)
        ...
        if (entries.length >= 2) {
            ...
            // Thời gian làm việc thực tế = last - first, trừ break overlap
            ...
            // Một log duy nhất → coi như không có mặt (actual_work_minutes = 0)
        }
        ...
        // Tính idle giữa các log (dùng hàm từ helpers)
        if (entries.length >= 2) {
            const timestamps = entries.map(e => e.ts);
        ...
        // Idle đầu ca / cuối ca
        if (firstLogMs || lastLogMs) {
            const edgeIdle = calcEdgeIdle(firstLogMs, lastLogMs, workStartMs, workEndMs);
            if (edgeIdle > 0) {
    ...
    const totalQcPerHour = totalActualMinutes
        ? Math.round((totalQcCount / (totalActualMinutes / 60)) * 10) / 10
        : 0;
...
async function getPeriodReport(startStr, endStr) {
    ...
    // Tính lại qc_per_hour cho từng user sau khi gộp
    ...
    if (type === 'daily') {
        ...
        if (report.needs_attention?.length) {
            ...
            report.needs_attention.slice(0, 3).forEach((u, i) => {
                msg += `${i + 1}. ${capitalizeName(u.name)} - idle ${u.total_idle_minutes}p (QC: ${u.qc_count}, ${u.qc_per_hour}/h)\n`;
            });
        }
    } else if (type === 'weekly') {
        ...
        msg += `👥 Xuất sắc (<${report.user_classification?.excellent_percent ? '?' : ''}): ${report.user_classification?.excellent_count} (${report.user_classification?.excellent_percent}%)\n`;
