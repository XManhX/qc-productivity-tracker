...
async function getDailyReport(dateStr, skipCache = false) {
    ...
    const activeCount = userDetails.filter(u => u.log_count > 0).length; // dùng để tính avg
    const avgIdlePerPerson =
        activeCount > 0 ? Math.round(total_idle_minutes / activeCount) : 0;
        ...
        summary: {
            ...
            percent_active:
                activeUsers.length > 0
                    ? Math.round((activeCount / activeUsers.length) * 100)
                    : 0,
...
async function getDailyReportForPeriod(startDateStr, endDateStr) {
    ...
    const totalSummary = days.reduce(
        (acc, day) => ({
            ...
            long_idle_events: acc.long_idle_events + (day.long_idle_events || 0),
        }),
        { total_work_hours: 0, total_idle_hours: 0, long_idle_events: 0 }
    );
...
async function getWeeklyReport(endDateStr, skipCache = false) {
    ...
    const currentIdleHours = thisWeek.summary?.total_idle_hours || 0;
    const lastIdleHours = lastWeek.summary?.total_idle_hours || 0;
    let improvementText;
    if (lastIdleHours === 0) {
        improvementText = currentIdleHours === 0 ? '0%' : 'N/A (tuần trước không có dữ liệu)';
    } else {
        const pct = Math.round(((lastIdleHours - currentIdleHours) / lastIdleHours) * 100);
        improvementText = `${pct}%`;
    }
    ...
    // long_idle_events cho tuần
    const thisWeekLongIdle = thisWeek.summary?.long_idle_events || 0;
    const lastWeekLongIdle = lastWeek.summary?.long_idle_events || 0;
    const alertReduction =
        lastWeekLongIdle > 0
            ? Math.round(((lastWeekLongIdle - thisWeekLongIdle) / lastWeekLongIdle) * 100)
            : 'N/A';
    ...
    const report = {
        summary: {
            ...
            alert_reduction: alertReduction, // dựa trên long_idle_events
        },
        user_classification: {
...
async function getMonthlyReport(year, month, skipCache = false) {
    ...
    let improvementText;
    if (lastIdleHours === 0) {
        improvementText = currentIdleHours === 0 ? '0%' : 'N/A (tháng trước không có dữ liệu)';
    } else {
        const pct = Math.round(((lastIdleHours - currentIdleHours) / lastIdleHours) * 100);
        improvementText = `${pct}%`;
    }
    ...
    if (
        (thisMonth.summary?.long_idle_events || 0) > (lastMonth.summary?.long_idle_events || 0)
    ) {
...
function buildReportMessage(report, type) {
    ...
    if (type === 'daily') {
        ...
        message += `⏱️ Số lần idle >30p: ${s.long_idle_events}\n\n`;
...
async function sendReportToSeatalk(report, type) {
    if (!report.summary || report.summary.total_qc === 0) {
...
function generateExcelBuffer(report, type) {
    ...
    const summaryWS = XLSX.utils.json_to_sheet([
        ...
        { 'Chỉ số': 'Tổng giờ làm việc', 'Giá trị': report.summary.total_work_hours },
        { 'Chỉ số': 'Tổng giờ idle', 'Giá trị': report.summary.total_idle_hours },
        { 'Chỉ số': 'Tỷ lệ idle', 'Giá trị': `${report.summary.idle_percent}%` },
        { 'Chỉ số': 'Trung bình idle/QC', 'Giá trị': `${report.summary.avg_idle_per_person} phút` },
        { 'Chỉ số': 'Số lần idle >30p', 'Giá trị': report.summary.long_idle_events },
    ]);
    XLSX.utils.book_append_sheet(wb, summaryWS, 'Tổng quan');

    const idleBreakdownWS = XLSX.utils.json_to_sheet([
        { 'Loại idle': 'Dưới 15 phút', 'Số lần': report.idle_breakdown.short_idle },
        { 'Loại idle': '15-30 phút', 'Số lần': report.idle_breakdown.medium_idle },
        { 'Loại idle': 'Hơn 30 phút', 'Số lần': report.idle_breakdown.long_idle },
    ]);
