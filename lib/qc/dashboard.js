...
export default async function handler(req, res) {
    ...
    try {
        ...
        const hourStartForStats = hs ?? 0;
        const hourEndForStats = he ?? 23;
        let totalOrders = 0;
        ...
        for (const r of results) {
            totalOrders += r.total;
            if (r.total > 0) activeQCs++;
            for (let h = hourStartForStats; h <= hourEndForStats; h++) {
                if (r.hourly[h] > 0) totalActiveHours++;
            }
        }
        const avgPerf = totalActiveHours > 0 ? Math.round(totalOrders / totalActiveHours) : 0;
        ...
        return res.status(200).json({
            success: true,
            date: targetDate,
            items: results.slice(startIdx, startIdx + pageSize),
            total: totalCount,
            stats: {
                totalOrders,
                activeQCs,
