...
export class HeatmapTable {
    ...
    sorted.forEach((user) => {
      ...
      tr.innerHTML = `<td class="px-4 py-2 text-left font-medium text-slate-900 border-r border-slate-100 max-w-[260px]">
        <div class="flex items-center gap-3 min-w-0">
          <div class="w-9 h-9 rounded-full bg-slate-100 flex items-center justify-center text-xs font-bold text-slate-600 uppercase border border-slate-200 flex-shrink-0">${(user.name || user.email).substring(0, 2)}</div>
          <div class="min-w-0">
  ...
  _buildHeader(headerRow, hourStart, hourEnd) {
    // Xóa cột giờ cũ (giữ lại 3 cột đầu: Nhân viên, Role, Tổng)
    ...
    if (headerRow.children.length === 0) {
      ...
      headerRow.innerHTML = `
        <th class="px-4 py-2 text-left font-semibold text-slate-700 hover:bg-slate-200/70" style="min-width: 220px">
          <button class="group flex items-center gap-1 w-full text-left" data-sort-trigger="name"><span>Nhân viên</span><span data-sort-icon="name"><i data-lucide="chevrons-up-down" class="w-3.5 h-3.5 text-slate-400"></i></span></button>
        </th>
        <th class="px-4 py-2 text-left font-semibold text-slate-700 hover:bg-slate-200/70" style="min-width: 90px">
          <button class="group flex items-center gap-1 w-full text-left" data-sort-trigger="role"><span>Role</span><span data-sort-icon="role"><i data-lucide="chevrons-up-down" class="w-3.5 h-3.5 text-slate-400"></i></span></button>
        </th>
        <th class="px-4 py-4 bg-emerald-50 font-bold text-emerald-800 hover:bg-emerald-100" style="min-width: 90px">
          <button class="group flex items-center justify-center gap-1 w-full" data-sort-trigger="total"><span>Tổng</span><span data-sort-icon="total"><i data-lucide="chevrons-up-down" class="w-3.5 h-3.5 text-slate-400"></i></span></button>
        </th>
      `;
    }
    ...
    for (let h = hourStart; h <= hourEnd; h++) {
      ...
      th.className =
        "px-3 py-4 text-slate-700 font-semibold border-l border-slate-200/50";
      th.innerHTML = `<button class="group flex items-center justify-center gap-1 w-full" data-sort-trigger="hour-${h}">
        <span>${h}h</span>
